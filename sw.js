...
const CACHE = 'fishcast-v21';
const ASSETS = ['./', './index.html', './m.html', './engine.js', './wx.js', './solunar.js', './bite.js', './photos.js', './backup.js', './i18n.js', './access.html', './guida.html', './privacy.html', './manifest.webmanifest', './icon-192.png', './icon-512.png'];
self.addEventListener('install', e => {
  e.waitUntil(caches.open(CACHE).then(c => c.addAll(ASSETS)).then(() => self.skipWaiting()));
});
self.addEventListener('activate', e => {
  e.waitUntil(caches.keys().then(ks => Promise.all(ks.filter(k => k !== CACHE && k !== 'fishcast-tiles').map(k => caches.delete(k)))).then(() => self.clients.claim()));
});
